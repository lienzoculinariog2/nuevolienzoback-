...
export class FileUploadService {
  ...
  async uploadImage(file: Express.Multer.File, productId: string): Promise<Products | null> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Error al actualizar URL de imagen en la base de datos');
    }
  ...
  async uploadImageForCategory(file: Express.Multer.File) {
    ...
    try {
      ...
      throw new InternalServerErrorException(`Error al subir imagen de categoría: ${error.message}`);
    }
  }
}

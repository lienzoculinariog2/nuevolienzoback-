...
export class FileUploadRepository {
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        ...
        (error, result) => {
          if (error) {
            ...
            return reject(
              new InternalServerErrorException(`Error al subir imagen: ${error.message}`),
            );
          }
          ...
          if (!result) {
            ...
            return reject(
              new InternalServerErrorException('Error: Cloudinary no retornó resultado'),
            );
          }
  ...
  private validateCloudinaryConfig(): void {
    ...
    if (missingVars.length > 0) {
      this.logger.error(
        `❌ Variables de entorno de Cloudinary faltantes: ${missingVars.join(', ')}`,
      );
      this.logger.warn(
        '⚠️ La subida de imágenes no funcionará. Configure las variables en Render.',
      );

...
console.log('🔍 PRUEBA SIMPLIFICADA DE WEBHOOK');
console.log('==================================\n');
...
async function testWebhookSimple() {
  try {
    ...
    // Crear un payment intent de prueba con configuración automática
    ...
    // Simular un pago exitoso usando una tarjeta de prueba
    console.log('\n💰 Simulando pago exitoso...');
    const confirmedPaymentIntent = await stripe.paymentIntents.confirm(paymentIntent.id, {
      payment_method_data: {
        type: 'card',
        card: {
          token: 'tok_visa' // Token de tarjeta de prueba de Stripe
        }
      }
    });
    ...
    console.log('\n⏳ Esperando 10 segundos para que el webhook se procese...');
    ...
    if (recentEvent) {
      ...
      if (recentEvent.pending_webhooks > 0) {
        ...
        console.log('   💡 Esto indica que el webhook no se pudo entregar');
      } else {
    ...
    if (recentEvent && recentEvent.pending_webhooks === 0) {
      console.log('✅ Webhook entregado exitosamente');
    } else {
      console.log('⚠️  Webhook no se pudo entregar - verificar configuración');
    }
    ...
    console.log('3. Si no hay logs, el problema está en la configuración del servidor');
    ...
    console.log('\n🔧 SI NO HAY LOGS EN RENDER:');
    console.log('1. Verificar que el servidor esté funcionando');
    console.log('2. Verificar que la URL del webhook sea correcta');
    console.log('3. Verificar que el endpoint /payments/webhook esté disponible');

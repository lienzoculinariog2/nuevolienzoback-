...
console.log('⏰ ESPERANDO A QUE EL DEPLOY SE COMPLETE');
console.log('========================================\n');
...
async function checkServerStatus(attempt = 1) {
  try {
...
async function waitForDeploy() {
  console.log('🚀 Iniciando monitoreo del deploy...');
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    const isReady = await checkServerStatus(attempt);
...
async function testWebhookOnceReady() {
  ...
  try {
    ...
    const testPayload = {
      id: 'evt_test_deploy_complete',
      object: 'event',
      api_version: '2025-07-30.basil',
      created: Math.floor(Date.now() / 1000),
      data: {
        object: {
          id: 'pi_test_deploy_complete',
